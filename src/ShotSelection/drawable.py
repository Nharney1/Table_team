...
from constants import Constants, BallColor
...
class Drawable:
    ...
    def __init__(self, body:b2Body, color:Tuple[int, int, int], draw:Callable[[b2Shape, b2Body, Tuple[int, int, int], ScreenInfo, bool, Tuple[int, int, int]], None], outline:bool=True, outline_color:Tuple[int, int, int]=BallColor.WHITE):
        self.body = body
        self.color = color
        self.draw_func = draw
    ...
    def draw_pool_cue(screen:ScreenInfo, cue_ball_pos, angle, shot_ready):
        ...
        if shot_ready:
            ...
            filename = os.path.join(dirname, './images/poolPlayer.png')
            cue_img = pygame.image.load(filename)
    ...
    def draw_billiard_ball_helper(position, r, screen:ScreenInfo, color, outline_color, num, angle):
        ...
        if num > 8:
            pygame.draw.circle(screen.screen, color, position, r)
            pygame.draw.circle(screen.screen, BallColor.WHITE, position, r * 0.90)
        ...
        # solids
            ...
            pygame.draw.circle(screen.screen, outline_color, position, r)
            pygame.draw.circle(screen.screen, color, position, r * 0.90)
        pygame.draw.circle(screen.screen, BallColor.WHITE, position, r / 3)