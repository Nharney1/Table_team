...
def verifyShotReachable(shot, balls):
...
def checkClearPath(cue_bal_pos, angle, balls):
    ...
    for i in range(16):
        ...
        unit_vector = (cos(radians(current_angle)), -sin(radians(current_angle)))
...
def lineSweep(start_seg, stop_seg, angle, angle_increase):
    ...
    cue_stick_peak = sqrt(pow(dif_x, 2) + pow(dif_y, 2))
    
    angle_length = cue_stick_peak / cos(radians(angle_increase))
    
    increased_angle = angle + 180 + angle_increase
   
    
    unit_vector = (cos(radians(increased_angle)), sin(radians(increased_angle)))
